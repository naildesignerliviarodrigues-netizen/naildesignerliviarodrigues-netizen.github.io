// v49-lembrete-concluido-historico
const services = [
    ...
    { id: "esmaltacao-plastica-pes", name: "Esmaltação em gel + Plástica dos pés", price: 90, durationText: "2h", durationMinutes: 120 },
...
function slotStatus(time){
    ...
    if(isManuallyBlockedSlot(selectedDateBR, start, end)){
        return { status: "unavailable", label: "BLOQUEADO" };
    }
...
function normalizeBusinessSettings(data){
    ...
    const days = Number(source.daysAhead);
    return {
        start: validTime(source.start) ? source.start : BUSINESS_DEFAULTS.start,
        weekdayEnd: validTime(source.weekdayEnd) ? source.weekdayEnd : BUSINESS_DEFAULTS.weekdayEnd,
        saturdayEnd: validTime(source.saturdayEnd) ? source.saturdayEnd : BUSINESS_DEFAULTS.saturdayEnd,
        lunchEnabled: source.lunchEnabled === undefined ? BUSINESS_DEFAULTS.lunchEnabled : Boolean(source.lunchEnabled),
        lunchStart: validTime(source.lunchStart) ? source.lunchStart : BUSINESS_DEFAULTS.lunchStart,
        lunchEnd: validTime(source.lunchEnd) ? source.lunchEnd : BUSINESS_DEFAULTS.lunchEnd,
        daysAhead: Number.isFinite(days) && days >= 1 && days <= 60 ? Math.round(days) : BUSINESS_DEFAULTS.daysAhead
    };
}

function updateBusinessHoursDisplay(){
    const weekday = document.getElementById("businessHoursWeekday");
    const lunch = document.getElementById("businessHoursLunch");
    const saturday = document.getElementById("businessHoursSaturday");
    if(weekday) weekday.textContent = `Terça a sexta • ${business.start} às ${business.weekdayEnd}`;
    if(lunch) lunch.textContent = business.lunchEnabled
        ? `Intervalo • ${business.lunchStart} às ${business.lunchEnd}`
        : "Atendimento direto, sem pausa para almoço";
    if(saturday) saturday.textContent = `Sábado • ${business.start} às ${business.saturdayEnd}`;
}

function applyBusinessSettings(data){
    Object.assign(business, normalizeBusinessSettings(data));
    renderBusinessSettingsAdmin();
    updateBusinessHoursDisplay();
    if(selectedService && selectedDateObj && screens.slots?.classList.contains("active")) renderTimes();
}

async function fetchBusinessSettingsFromFirebase(){
    try{
        const response = await fetch(firebaseUrl("settings/business"), { cache: "no-store" });
        if(!response.ok) throw new Error(`Firebase settings HTTP ${response.status}`);
        const data = await response.json();
        applyBusinessSettings(data || BUSINESS_DEFAULTS);
    }catch(error){
        console.warn("Configuração de horários indisponível; usando padrão:", error);
...
async function saveBusinessSettingsToFirebase(settings){
    ...
    if(!response.ok) throw new Error(`Firebase settings PUT ${response.status}`);
    applyBusinessSettings(normalized);
    return normalized;
}

function ensureBusinessSettingsAdmin(){
    const adminScreen = document.getElementById("adminScreen");
    const photoBtn = document.getElementById("btnAdminPhotos");
    if(!adminScreen || !photoBtn) return;

    if(!document.getElementById("businessSettingsStyle")){
        const style = document.createElement("style");
        style.id = "businessSettingsStyle";
        style.textContent = `
            .business-settings-card{background:var(--card);border:1px solid var(--border);border-radius:24px;padding:16px;margin:0 0 12px}
            .business-settings-card h2{color:var(--pink);font-size:20px;margin:0 0 7px}
            .business-settings-card .settings-help{color:var(--muted);font-size:14px;line-height:1.35;margin:0 0 14px}
            .business-settings-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
            .business-setting-field{display:grid;gap:6px}
            .business-setting-field.full{grid-column:1/-1}
            .business-setting-field label{font-size:13px;color:#f4dce7;font-weight:800}
            .business-setting-field input{width:100%;height:52px;border:0;border-radius:16px;background:#fff0f6;color:#080408;padding:0 12px;font-size:15px;font-weight:800;outline:none;box-sizing:border-box}
            .business-lunch-toggle{grid-column:1/-1;display:flex;align-items:center;gap:10px;background:#090507;border:1px solid rgba(255,105,180,.20);border-radius:16px;padding:12px}
            .business-lunch-toggle input{width:22px;height:22px;accent-color:#e45a9d}
            .business-lunch-toggle span{font-size:14px;font-weight:800;color:#fff}
            .business-save-btn{width:100%;min-height:54px;border:0;border-radius:18px;background:linear-gradient(135deg,#e45a9d,#ff78bb);color:#14000b;font-weight:900;font-size:15px;margin-top:12px}
            .business-settings-note{font-size:12px;color:var(--muted);line-height:1.35;margin:10px 0 0}
        `;
        document.head.appendChild(style);
    }

    let card = document.getElementById("businessSettingsCard");
    if(!card){
        card = document.createElement("section");
        card.id = "businessSettingsCard";
        card.className = "business-settings-card";
        card.innerHTML = `
            <h2>Horários do estúdio</h2>
            <p class="settings-help">Altere aqui o funcionamento e o almoço. A agenda das clientes se reorganiza automaticamente.</p>
            <div class="business-settings-grid">
                <div class="business-setting-field"><label>Abertura</label><input type="time" id="businessStart"></div>
                <div class="business-setting-field"><label>Fechamento (terça a sexta)</label><input type="time" id="businessWeekdayEnd"></div>
                <div class="business-setting-field"><label>Fechamento sábado</label><input type="time" id="businessSaturdayEnd"></div>
                <div class="business-setting-field"><label>Dias futuros na agenda</label><input type="number" min="1" max="60" id="businessDaysAhead"></div>
                <label class="business-lunch-toggle"><input type="checkbox" id="businessLunchEnabled"><span>Usar horário de almoço (terça a sexta)</span></label>
                <div class="business-setting-field"><label>Início do almoço</label><input type="time" id="businessLunchStart"></div>
                <div class="business-setting-field"><label>Fim do almoço</label><input type="time" id="businessLunchEnd"></div>
            </div>
            <button type="button" id="btnSaveBusinessSettings" class="business-save-btn">SALVAR HORÁRIOS</button>
            <p class="business-settings-note">Atendimentos que atravessariam o almoço ficam indisponíveis automaticamente. Sábado continua sem intervalo de almoço.</p>
        `;
        photoBtn.insertAdjacentElement("afterend", card);
        document.getElementById("btnSaveBusinessSettings").addEventListener("click", handleSaveBusinessSettings);
        document.getElementById("businessLunchEnabled").addEventListener("change", updateLunchFieldsState);
    }
    renderBusinessSettingsAdmin();
}

function updateLunchFieldsState(){
    const enabled = document.getElementById("businessLunchEnabled")?.checked ?? true;
    ["businessLunchStart", "businessLunchEnd"].forEach(id => {
...
function renderBusinessSettingsAdmin(){
    if(!document.getElementById("businessSettingsCard")) return;
    document.getElementById("businessStart").value = business.start;
    document.getElementById("businessWeekdayEnd").value = business.weekdayEnd;
    document.getElementById("businessSaturdayEnd").value = business.saturdayEnd;
    document.getElementById("businessDaysAhead").value = business.daysAhead;
    document.getElementById("businessLunchEnabled").checked = Boolean(business.lunchEnabled);
    document.getElementById("businessLunchStart").value = business.lunchStart;
    document.getElementById("businessLunchEnd").value = business.lunchEnd;
    updateLunchFieldsState();
}

function readBusinessSettingsAdmin(){
    return {
        start: document.getElementById("businessStart").value,
        weekdayEnd: document.getElementById("businessWeekdayEnd").value,
        saturdayEnd: document.getElementById("businessSaturdayEnd").value,
        daysAhead: Number(document.getElementById("businessDaysAhead").value),
        lunchEnabled: document.getElementById("businessLunchEnabled").checked,
        lunchStart: document.getElementById("businessLunchStart").value,
        lunchEnd: document.getElementById("businessLunchEnd").value
    };
}
...
function validateBusinessSettings(settings){
    ...
    if(!Number.isFinite(settings.daysAhead) || settings.daysAhead < 1 || settings.daysAhead > 60) return "Use entre 1 e 60 dias futuros na agenda.";
    return "";
}
...
async function handleSaveBusinessSettings(){
    ...
    try{
        ...
        console.warn("Erro salvando horários:", error);
        showToast("Não foi possível salvar os horários.", "error");
    }finally{
        btn.disabled = false;
...
function normalizeBlockedSlotFromFirebase(item){
    return {
        id: item.id || "",
        date: item.date || "",
        dateBR: item.dateBR || item.date_br || "",
        start: item.start || "",
        end: item.end || "",
...
function blockedSlotToFirebase(item){
    return {
        id: item.id,
        date: item.date || "",
        date_br: item.dateBR || "",
...
function saveBlockedSlotsLocal(items){
    blockedSlotsCache = Array.isArray(items) ? items : [];
    localStorage.setItem("livia_blocked_slots", JSON.stringify(blockedSlotsCache));
}

function loadBlockedSlotsLocal(){
    try{
        const data = JSON.parse(localStorage.getItem("livia_blocked_slots") || "[]");
        return Array.isArray(data) ? data : [];
    }catch(error){
        return [];
    }
...
function getStoredBlockedSlots(){
    return blockedSlotsCache.length ? blockedSlotsCache : loadBlockedSlotsLocal();
}

async function fetchBlockedSlotsFromFirebase(){
    const response = await fetch(firebaseUrl("blocked_slots"), { cache: "no-store" });
    if(!response.ok) throw new Error(`Firebase blocked_slots HTTP ${response.status}`);
    const data = await response.json();
    const items = normalizeFirebaseData(data).map(normalizeBlockedSlotFromFirebase);
    saveBlockedSlotsLocal(items);
    return items;
}

function loadBlockedSlotsFromFirebase(){
    fetchBlockedSlotsFromFirebase()
        .then(() => rerenderCurrentDataScreens())
        .catch(error => {
            console.warn("Bloqueios indisponíveis, usando backup local:", error);
        });
}

async function putBlockedSlotFirebase(item){
    const response = await fetch(firebaseUrl(`blocked_slots/${item.id}`), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(blockedSlotToFirebase(item))
    });
    if(!response.ok) throw new Error(`Firebase blocked_slots PUT ${response.status}`);
    return response.json();
}

async function deleteBlockedSlotFirebase(id){
    const response = await fetch(firebaseUrl(`blocked_slots/${id}`), { method: "DELETE" });
    if(!response.ok) throw new Error(`Firebase blocked_slots DELETE ${response.status}`);
    return response.json();
}

function isManuallyBlockedSlot(dateBR, start, end){
    const blocks = getStoredBlockedSlots().filter(block => {
        if(!block || !block.start || !block.end) return false;
        return block.dateBR === dateBR || block.date === selectedDate;
    });
...
function ensureAdminBlockPanel(){
    const adminScreen = document.getElementById("adminScreen");
    const photoBtn = document.getElementById("btnAdminPhotos");
    if(!adminScreen || !photoBtn) return;

    if(!document.getElementById("adminBlockStyle")){
        const style = document.createElement("style");
        style.id = "adminBlockStyle";
        style.textContent = `
            .admin-block-card{background:var(--card);border:1px solid var(--border);border-radius:24px;padding:16px;margin:0 0 12px}
            .admin-block-card h2{color:var(--pink);font-size:20px;margin:0 0 8px}
            .admin-block-card p{color:var(--muted);font-size:14px;line-height:1.35;margin:0 0 12px}
            .admin-block-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-bottom:9px}
            .admin-block-card input{width:100%;height:52px;border:0;border-radius:16px;background:#fff0f6;color:#080408;padding:0 12px;font-size:15px;font-weight:800;outline:none}
            .admin-block-card .admin-block-reason{grid-column:1 / -1}
            .admin-block-btn{width:100%;min-height:54px;border:0;border-radius:18px;background:linear-gradient(135deg,#e45a9d,#ff78bb);color:#14000b;font-weight:900;font-size:15px;margin-top:4px}
            .admin-block-list{display:grid;gap:9px;margin-top:12px}
            .admin-block-item{background:#090507;border:1px solid rgba(255,105,180,.20);border-radius:16px;padding:11px 12px;display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center}
            .admin-block-item strong{color:#fff;font-size:15px;display:block;margin-bottom:4px}
            .admin-block-item span{color:var(--muted);font-size:13px;line-height:1.25}
            .admin-unblock-btn{border:0;border-radius:14px;background:var(--darkBtn);color:#fff;font-weight:900;font-size:12px;min-height:42px;padding:0 12px}
        `;
        document.head.appendChild(style);
    }

    let card = document.getElementById("adminBlockCard");
    if(!card){
        card = document.createElement("section");
        card.id = "adminBlockCard";
        card.className = "admin-block-card";
        card.innerHTML = `
            <h2>Bloquear horário</h2>
            <p>Use quando precisar sair ou pausar a agenda. Os horários bloqueados não aparecem para as clientes.</p>
            <div class="admin-block-grid">
                <input type="date" id="adminBlockDate">
                <input type="time" id="adminBlockStart" value="09:00">
                <input type="time" id="adminBlockEnd" value="10:30">
                <input type="text" id="adminBlockReason" class="admin-block-reason" placeholder="Motivo opcional: saída urgente">
            </div>
            <button type="button" id="btnCreateBlockSlot" class="admin-block-btn">BLOQUEAR HORÁRIO</button>
            <div id="adminBlockList" class="admin-block-list"></div>
        `;
        photoBtn.insertAdjacentElement("afterend", card);
        document.getElementById("adminBlockDate").value = todayInputValue();
        document.getElementById("btnCreateBlockSlot").addEventListener("click", createAdminBlockedSlot);
    }
}

async function createAdminBlockedSlot(){
    const dateInput = document.getElementById("adminBlockDate");
    const startInput = document.getElementById("adminBlockStart");
    const endInput = document.getElementById("adminBlockEnd");
    const reasonInput = document.getElementById("adminBlockReason");

    const date = dateInput?.value || "";
    const start = startInput?.value || "";
    const end = endInput?.value || "";
    const reason = (reasonInput?.value || "Bloqueio manual").trim();

    if(!date){ alert("Escolha a data que deseja bloquear."); return; }
    if(!start || !end){ alert("Informe o horário de início e fim."); return; }
    if(timeToMinutes(end) <= timeToMinutes(start)){ alert("O horário final precisa ser maior que o inicial."); return; }

    const dateBR = formatDateInputToBR(date);
    const block = {
        id: `block-${Date.now()}`,
        date,
        dateBR,
    ...
    const items = getStoredBlockedSlots().filter(item => item.id !== block.id);
    items.unshift(block);
    saveBlockedSlotsLocal(items);
    renderAdminBlockedSlots();
    if(selectedDate === date || selectedDateBR === dateBR) renderTimes();

    try{
        await putBlockedSlotFirebase(block);
        await fetchBlockedSlotsFromFirebase();
        renderAdminBlockedSlots();
        if(selectedDate === date || selectedDateBR === dateBR) renderTimes();
        showToast("Horário bloqueado com sucesso ✓");
    }catch(error){
        console.warn("Erro salvando bloqueio:", error);
        showToast("Bloqueio salvo neste aparelho, mas ainda não sincronizou.", "error");
    }
}

async function removeAdminBlockedSlot(id){
    const block = getStoredBlockedSlots().find(item => item.id === id);
    const ok = await showPrettyConfirm({
        title: "Liberar horário?",
        message: "Esse intervalo voltará a aparecer como disponível se não existir outro agendamento no mesmo horário.",
        confirmText: "SIM, LIBERAR",
        cancelText: "VOLTAR"
    });
    if(!ok) return;

    const remaining = getStoredBlockedSlots().filter(item => item.id !== id);
    saveBlockedSlotsLocal(remaining);
    renderAdminBlockedSlots();

    if(block && (selectedDate === block.date || selectedDateBR === block.dateBR)) renderTimes();

    try{
        await deleteBlockedSlotFirebase(id);
        await fetchBlockedSlotsFromFirebase();
        renderAdminBlockedSlots();
        if(block && (selectedDate === block.date || selectedDateBR === block.dateBR)) renderTimes();
        showToast("Horário liberado ✓");
    }catch(error){
        console.warn("Erro removendo bloqueio:", error);
        showToast("Liberei no aparelho, mas houve falha ao sincronizar.", "error");
    }
}

function renderAdminBlockedSlots(){
    ensureAdminBlockPanel();
    const list = document.getElementById("adminBlockList");
    if(!list) return;

    const items = [...getStoredBlockedSlots()].sort((a,b) => {
        const da = new Date(a.date || "2999-12-31");
        const db = new Date(b.date || "2999-12-31");
        return (da - db) || (timeToMinutes(a.start || "00:00") - timeToMinutes(b.start || "00:00"));
    });

    if(items.length === 0){
        list.innerHTML = `<p class="admin-empty">Nenhum horário bloqueado.</p>`;
        return;
    }
    ...
    list.innerHTML = items.map(item => `
        <div class="admin-block-item">
            <div>
                <strong>${item.dateBR || item.date} • ${item.start} às ${item.end}</strong>
                <span>${item.reason || "Bloqueio manual"}</span>
            </div>
            <button type="button" class="admin-unblock-btn" data-block-id="${item.id}">LIBERAR</button>
        </div>
    `).join("");

    list.querySelectorAll(".admin-unblock-btn").forEach(btn => {
        btn.addEventListener("click", () => removeAdminBlockedSlot(btn.dataset.blockId));
    });
}
...
function statusClass(status){
    ...
    if(normalized.includes("concl")) return "confirmed";
...
function appointmentCard(item){
    return `
        <section class="appointment-card ${statusClass(item.status)}">
            ...
            ${!["Cancelado","Expirado","Concluído"].includes(item.status) ? `<button class="cancel-appointment-btn" data-id="${item.id}">CANCELAR AGENDAMENTO</button>` : ""}
        </section>
    `;
}
...
function isStatus(item, key){
    ...
    if(key === "completed") return s.includes("concl");
...
function appointmentHasEnded(item){
    if(!item || !item.date || !item.time) return false;

    const d = parseBRDateToDate(item.date);
    if(!d || isNaN(d.getTime())) return false;

    const service = services.find(s => s.id === item.serviceId || s.name === item.service);
    const duration = Number(item.durationMinutes || service?.durationMinutes || 60);
    const endMinutes = timeToMinutes(item.time || "00:00") + duration;

    d.setHours(Math.floor(endMinutes / 60), endMinutes % 60, 0, 0);
    return d.getTime() < Date.now();
}

function displayStatusForItem(item){
    const status = String(item?.status || "");
    const normalized = status.toLowerCase();

    if((normalized.includes("confirm") || normalized === "pago") && appointmentHasEnded(item)){
        return "Concluído";
    }
...
function ensureAdminReminderStyle(){
    ...
    style.textContent = `
        ...
        .admin-whatsapp-btn{background:#22a866!important;color:#06170d!important;}
        .admin-reminder-btn{background:linear-gradient(135deg,#e45a9d,#ff78bb)!important;color:#14000b!important;}
...
function adminBookingCard(item){
    ...
    const canCancel = !isStatus(item, "cancelled") && !isStatus(item, "expired") && !isStatus(item, "completed");
    const canRemind = phoneDigits && !isStatus(item, "cancelled") && !isStatus(item, "expired") && !isStatus(item, "completed");
        ...
        <section class="admin-booking-card ${statusClass(status)}">
            ...
            ${phoneDigits ? `
                <div class="admin-contact-actions">
                    ...
                    ${canRemind ? `<button class="admin-reminder-btn" data-whatsapp="${reminderLink}">LEMBRETE</button>` : `<button class="admin-reminder-btn" disabled style="opacity:.45!important;">CONCLUÍDO</button>`}
                </div>
            ` : ""}
...
function updateAppointmentStatus(id, status){
    ...
    const items = getStoredAppointments().map(item => {
        if(item.id === id){
            ...
            if(status === "Concluído"){
                ...
                updated.paymentStatus = "paid_completed";
            }
    ...
    if(status === "Concluído"){
        patch.payment_status = "paid_completed";
        patch.completed_at = new Date().toLocaleString("pt-BR");
    }
    if(status === "Reserva temporária"){
...
function updateFinanceDetailLine({confirmed, completed = [], temp, expired, cancelled, confirmedTotal, tempTotal}){
    const line = document.getElementById('adminFinanceLine');
    if(!line) return;
    line.classList.add('admin-finance-detail');
    line.innerHTML = `
        <span><b>${confirmed.length}</b> confirmado(s) • <b>${completed.length}</b> concluído(s) • <b>${money(confirmedTotal)}</b> recebidos</span>
...
function renderAdmin(){
    ...
    const completed = items.filter(item => isStatus(item, "completed"));
    const paidItems = [...confirmed, ...completed];
    ...
    const confirmedToday = paidItems.filter(item => isSameBRDate(item.date, today));
    const confirmedWeek = paidItems.filter(item => isWithinNextDays(item.date, 7));
    const confirmedMonth = paidItems.filter(item => currentMonthMatch(item.date));

    const sumDeposits = arr => arr.reduce((acc,item) => acc + Number(item.deposit || 0), 0);
    const totalConfirmedDeposits = sumDeposits(paidItems);
    ...
    document.getElementById("adminSignalsLine").textContent =
        `Sinais recebidos: ${money(sumDeposits(paidItems))} | Sinais temporários: ${money(sumDeposits(temp))}`;

    const todayItems = sortAppointments(items.filter(item => isSameBRDate(item.date, today) && (isStatus(item, "confirmed") || isStatus(item, "temporary") || isStatus(item, "completed"))));
    document.getElementById("adminTodayLine").textContent =
        `${formatDateBR(today)} • ${todayItems.length} atendimento(s) • ${money(sumDeposits(todayItems.filter(i => isStatus(i, "confirmed") || isStatus(i, "completed"))))} recebidos`;
